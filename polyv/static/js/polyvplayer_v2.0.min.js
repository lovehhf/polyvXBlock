(function(a, b) {
    ...
    f = function() {
        ...
        e.extend({
            noConflict: function(b) {
                a.$ === e && (a.$ = g),
                b && a.polyvObject === e && (a.polyvObject = f);
                return e
            },
            isReady: !1,
            readyWait: 1,
    ...
    f.extend({
        cache: {},
        uuid: 0,
        expando: "polyvObject" + (f.fn.jquery + Math.random()).replace(/\D/g, ""),
        noData: {
            embed: !0,
...
var swfobject = function() {
...
} (); (function(polyvObject) {
    polyvObject.majorVersion = 10;
    polyvObject.majorRevision = 3;
    createPreviewPlayer = function(options) {
        options.previewMode = true;
        return privateCreate(this, options)
    };
    createShowPlayer = function(options) {
        function getPosition(ele) {
            var arr = [];
            var parents = polyvObject(ele).parents();
            for (var i = 0; i < parents.length; i++) {
                arr.push(polyvObject(parents[i]).css("position"))
            }
            return arr
        }
        function resetPosition(ele, arr) {
            var parents = polyvObject(ele).parents();
            for (var i = 0; i < parents.length; i++) {
                polyvObject(parents[i]).css("position", arr[i])
            }
        }
        ...
        if (polyvObject.isSupportedHTMLDevice() || options.forceHTML5) {
            var cdnURL = "http://v3.polyv.net";
            var container = this;
            var settings = options;
            var href = window.location.href;
            var vid = options.vid;
            var idName = "polyvPlayer" + vid;
            var playStart = false;
            var isScale = false;
            var initWidth = settings.width;
            var ratio = 1;
            var duration = 0;
            var currentTime = 0;
            var stayInVideoTime = 0;
            var isFullScreen = false;
            polyvObject(this).css("height", settings.height);
            polyvObject(this).css("width", settings.width);
            var playerUrl = "http://static.polyv.net/file/player/view.html" + "?href=" + href + "&settings=" + encodeURI(jsonStr);
            polyvObject("<iframe />", {
                name: "polyvPlayer",
                id: idName,
                frameborder: "0",
            ...
            if (settings.height == 0) {
                ...
                polyvObject.ajax({
                    context: this,
                    url: cdnURL + "/videojson/" + vid + ".js",
                    type: "GET",
                    dataType: "jsonp",
                    async: false,
                    success: function(data) {
                        settings.ratio = data.ratio;
                        ratio = settings.ratio;
                        var videoheight = parseInt(polyvObject(this).css("width")) / settings.ratio;
                        settings.height = videoheight;
                        polyvObject(this).css("height", videoheight);
                        preHeight = polyvObject(this).css("height")
                    }
                })
            }
            settings.j2s_getDuration = function() {
                return duration
            };
            settings.j2s_getCurrentTime = function() {
                return currentTime
            };
            settings.j2s_resumeVideo = function() {
                send("j2s_resumeVideo", vid)
            };
            settings.j2s_pauseVideo = function() {
                send("j2s_pauseVideo", vid)
            };
            settings.j2s_seekVideo = function(t) {
                send("j2s_seekVideo", vid, t)
            };
            settings.j2s_stopVideo = function() {
                send("j2s_stopVideo", vid)
            };
            settings.j2s_stayInVideoTime = function() {
                return stayInVideoTime
            };
            document.getElementById(idName).addEventListener("touchend",
            function(e) {
                send("touchEnd", vid)
            },
            false);
            var position = polyvObject(this).css("position");
            var preWidth = settings.width;
            var preHeight = settings.height;
            var preObjects = getPosition(this);
            window.addEventListener("message",
            function(event) {
                if (event.data.vid == vid) {
                    switch (event.data.message) {
                    case "fullScreen":
                        isFullScreen = true;
                        polyvObject("#" + idName).css({
                            position: "absolute",
                            "z-index": 1001,
                            top: 0,
                            left: 0
                        });
                        polyvObject(container).css({
                            position: "static",
                            width: "100%",
                            height: "100%"
                        });
                        polyvObject(container).parents().css("position", "static");
                        send("adjust", vid);
                        break;
                    case "normalScreen":
                        isFullScreen = false;
                        ...
                        resetPosition(container, preObjects);
                        send("adjust", vid);
                        break;
                    case "s2j_onPlayStart":
                        duration = event.data.duration;
                        if (settings.s2j_onPlayStart) {
                            playStart = true;
                            settings.s2j_onPlayStart()
                        }
                    ...
                    case "s2j_onVideoPlay":
                        if (settings.s2j_onVideoPlay) {
                            settings.s2j_onVideoPlay()
                        }
                    ...
                    case "s2j_onVideoPause":
                        if (settings.s2j_onVideoPause) {
                            settings.s2j_onVideoPause()
                        }
                        if (typeof s2j_onVideoPause == "function") {
                            s2j_onVideoPause(vid)
                        }
                        break;
                    case "s2j_onPlayOver":
                        if (settings.s2j_onPlayOver) {
                            settings.s2j_onPlayOver()
                        }
                        ...
                        break;
                    case "renderParam":
                        currentTime = event.data.cur;
                        stayInVideoTime = event.data.stay;
                        break
                    }
                }
            },
            ...
            function() {
                setTimeout(function() {
                    if (isScale) {
                        if (initWidth.indexOf("%") != -1) {
                            preWidth = parseInt(initWidth) / 100 * polyvObject(window).width();
                            preHeight = preWidth / ratio;
                            preWidth = initWidth
                        } else {
                            preHeight = parseInt(preWidth) / ratio
                        }
                        if (!isFullScreen) {
                            polyvObject("#" + idName).css({
                                position: "relative"
            ...
            false);
            function send(str, value, time) {
                if (!arguments[2]) time = 0;
                document.getElementById(idName).contentWindow.postMessage({
                    value: str,
                    vid: value,
                    seekT: time
                },
    ...
    privateCreate = function(container, options) {
        var statTimer;
        var cdnURL = "http://v3.polyv.net";
        var resourceURL = "http://v.polyv.net";
        setCookie = function(cname, cvalue, exdays) {
            var d = new Date;
            d.setTime(d.getTime() + exdays * 24 * 60 * 60 * 1e3);
            var expires = "expires=" + d.toGMTString();
            document.cookie = cname + "=" + cvalue + "; " + expires + ";domain=.polyv.net;path=/"
        };
        polyvObject.renderInstallGif = function(container, playerType) {
            if (container.css("height").charAt(container.css("height").length - 1) == "%") {
                container.css("display", "block")
            } else {
                container.css("display", "inline-block")
            }
            var imgUrl = "";
            if (playerType == polyvObject.playerType.INSTALLER) {
                imgUrl = resourceURL + "/file/images/install_flash_player.gif"
            } else {
                imgUrl = resourceURL + "/file/images/upgrade_flash_player.gif"
            }
            var linkHTML = "<a href='http://www.adobe.com/go/getflash/' target='_blank'><img src='" + imgUrl + "' alt='Get Flash Player' width='314' height='200' border='0'></a>";
            container.html(linkHTML)
        };
        polyvObject.launchFullScreen = function(element) {
        ...
        polyvObject.makeFlashPlayer = function(container, settings) {
            var isMobile = polyvObject.isSupportedHTMLDevice();
            if (window.location.protocol != "https:") {
                settings.swf_link = "http://" + settings.domain + "/videos/player.swf"
            } else {
                settings.swf_link = "https://" + settings.secureDomain + "/videos/player.swf"
            }
            if (!settings.flashvars) {
                settings.flashvars = {}
            }
            settings.flashvars.vid = settings.vid;
            if (settings.previewMode == true) {
                settings.flashvars.watchEndTime = settings.previewlong
            }
            if (isMobile) {
                settings.flashvars.banSaveSegData = "on"
            }
            for (var p in settings.params) {
                settings.flashvars[p] = settings.params[p]
            }
            var varstring = "";
            for (var oneVar in settings.flashvars) {
                varstring = varstring + oneVar + "=" + settings.flashvars[oneVar] + "&"
            }
            for (var p in settings.params) {
                settings.flashvars[p] = settings.params[p]
            }
            if (polyvObject.isIE) {
                settings.flashParams.movie = settings.swf_link;
                var options = "";
                options += '<param name="flashvars" value="' + varstring + '" />';
                for (var pOption in settings.flashParams) {
                    options += '<param name="' + pOption + '" value="' + settings.flashParams[pOption] + '" />'
                }
                var flashHTML = '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" id="' + settings.vid + '"' + ' width="' + settings.width + '"' + ' height="' + settings.height + '"' + ' type="application/x-shockwave-flash"' + ' class="polyvFlashObject">' + options + "</object>";
                container.html(flashHTML)
            } else {
                var flashElement = document.createElement("object");
                flashElement.type = "application/x-shockwave-flash";
                flashElement.data = settings.swf_link;
                flashElement.id = settings.vid;
                flashElement.width = settings.width;
                flashElement.height = settings.height;
                flashElement.className = "polyvFlashObject";
                settings.flashParams.flashvars = varstring;
                var tempParam;
                for (var config in settings.flashParams) {
                    tempParam = document.createElement("param");
                    tempParam.name = config;
                    tempParam.value = settings.flashParams[config];
                    flashElement.appendChild(tempParam)
                }
                container.html(flashElement)
            }
        };
        polyvObject.renderFlash = function(container, settings) {
            if (settings.height == 0) {
                polyvObject.ajax({
                    url: cdnURL + "/videojson/" + settings.vid + ".js",
                    type: "GET",
                    dataType: "jsonp",
                    async: false,
                    success: function(data) {
                        settings.ratio = data.ratio;
                        if (settings.width.charAt(settings.width.length - 1) == "%") {
                            var width = window.innerWidth > 0 ? window.innerWidth: screen.width;
                            settings.height = width * (parseInt(settings.width) / 100) / data.ratio
                        } else {
                            settings.height = settings.width / data.ratio
                        }
                        polyvObject.makeFlashPlayer(container, settings)
                    }
                })
            } else {
                polyvObject.makeFlashPlayer(container, settings)
        ...
        containInHost = function(host_config) {
            var hosts = host_config.split(",");
            for (var i = 0; i < hosts.length; i++) {
                if (hosts[i].length > 0 && window.location.href.indexOf(hosts[i]) != -1) {
                    return true
                }
            }
            return false
        };
        var settingLinkBase = cdnURL + "/uc/video/settings";
        var settings = {
            flashParams: {
        ...
        settings.previewlong = 180;
        if (options) {
            polyvObject.extend(settings, options)
        }
        if (!settings.vid) {
            return
        }
        var lowerDevice = polyvObject.checkLowerDevice();
        var flashSupport = polyvObject.checkFlashSupport();
        var htmlSupport = polyvObject.checkHtmlSupport();
        var playerType = polyvObject.determinePlayerType(settings, flashSupport, htmlSupport);
        var userid = settings.vid.substring(0, 10);
        if (playerType == polyvObject.playerType.NO_SUPPORT || playerType == polyvObject.playerType.INSTALLER) {
            polyvObject.renderInstallGif(container, playerType);
            return
        }
        if (playerType == polyvObject.playerType.FLASH) {
            polyvObject.renderFlash(container, settings);
            if (settings.s2j_onPlayStart) {
                s2j_onPlayStart = settings.s2j_onPlayStart
            }
            if (settings.s2j_onPlayOver) {
                s2j_onPlayOver = settings.s2j_onPlayOver
            }
            if (settings.s2j_onVideoPlay) {
                s2j_onVideoPlay = settings.s2j_onVideoPlay
            }
            if (settings.s2j_onVideoPause) {
                s2j_onVideoPause = settings.s2j_onVideoPause
            }
            if (settings.s2j_onPlayerInitOver) {
                s2j_onPlayerInitOver = settings.s2j_onPlayerInitOver
            }
            if (settings.s2j_focusComment) {
                s2j_focusComment = settings.s2j_focusComment
            }
            return polyvObject.getPlayer(settings.vid)
        }
        var hash = "";
        polyvObject.ajax({
            url: cdnURL + "/videojson/" + settings.vid + ".js",
            ...
            success: function(data) {
                var outdate = true;
                if (data.outflow == "false" && data.timeoutflow == "false") {
                    outdate = false
                }
                hash = data.hash;
                if (data.setting_type == 1 && containInHost(data.disable_host)) {
                    outdate = true
                }
                if (data.setting_type == 2 && !containInHost(data.enable_host) && window.location.href.indexOf("polyv.net") == -1) {
                    outdate = true
                }
                settings.outdate = outdate;
                settings.zColor = data.player.zColor;
                settings.skincolor = data.player.skincolor;
                settings.pColor = data.player.pColor;
                settings.videolink = data.videolink;
                settings.my_br = data.my_br;
                settings.flv = data.flv;
                settings.mp4 = data.mp4;
                settings.hls = data.hls;
                settings.df_num = data.df_num;
                settings.hlsIndex = data.hlsIndex;
                settings.duration = data.duration;
                settings.j2s_getDuration = function() {
                    return settings.duration
                };
                settings.first_image = data.first_image;
                settings.seed = data.seed;
                settings.ratio = data.ratio;
                settings.swf_link = data.swf_link;
                settings.status = data.status;
                if (data.status < 60) {
                    settings.outdate = true
                }
                makeH5Player(container)
            }
        });
        function makeVideo(o, video) {
            var polyvObjectvideo = polyvObject(video);
            var imageUrl = settings.first_image;
            if (settings.flashvars["loading_bg_img"]) {
                imageUrl = settings.flashvars["loading_bg_img"]
            }
            if (settings.loading_bg_img.length > 0) {
                imageUrl = settings.loading_bg_img
            }
            if (settings.seed == 1 || settings.forceHLS == true) {
                sourceType = "hls";
                if (settings.forceHightDf == "true") {
                    video.src = settings.hls[settings.df_num - 1]
                } else {
                    if (settings.df != 0) {
                        if (settings.df <= settings.df_num) {
                            video.src = settings.hls[settings.df - 1]
                        } else {
                            video.src = settings.hls[0]
                        }
                    } else {
                        video.src = settings.hlsIndex
                    }
                }
                if (settings.ts != "" && settings.sign != "") {
                    video.src = video.src + "?ts=" + settings.ts + "&sign=" + settings.sign
                }
                if (settings.previewMode == true) {
                    var vpid = settings.vid.substring(0, 32);
                    video.src = video.src.replace(vpid, "p_" + vpid)
                }
            } else {
                if (settings.forceHightDf == "true") {
                    video.src = settings.mp4[settings.df_num - 1];
                    currentBr = settings.df_num - 1
                } else {
                    video.src = settings.mp4[settings.my_br - 1];
                    currentBr = settings.my_br
                }
                if (lowerDevice) {
                    video.src = settings.mp4[0];
                    currentBr = 0
                }
                sourceType = "mp4"
            }
            video.preload = "auto";
            if (settings.hidecontrol != "true" || settings.hidecontrol == undefined) {
                video.controls = "controls"
            }
            var ban_seek_by_limit_time = settings.flashvars["ban_seek_by_limit_time"];
            if (ban_seek_by_limit_time && ban_seek_by_limit_time == "on") {
                if (video.hasAttribute("controls")) {
                    video.removeAttribute("controls")
                }
            }
            video.id = settings.vid;
            settings.stay_duration = 0;
            video.poster = imageUrl;
            polyvObjectvideo.css("background-size", settings.playerWidth * 100 + "%");
            polyvObjectvideo.css("background-repeat", "no-repeat");
            polyvObjectvideo.css("background-position", "center");
            polyvObjectvideo.css("width", settings.width);
            polyvObjectvideo.css("height", settings.height);
            polyvObject(o).css("background-image", "");
            polyvObjectvideo.css("display", "block");
            video.play()
        }
        function countStayDuration() {
            settings.stay_duration++
        }
        function sendStat() {
            var ts = (new Date).getTime();
            var plain = "rtas.net" + settings.pid + settings.vid + "0" + settings.stay_duration;
            var sign = CryptoJS.MD5(plain) + "";
            var data = {
                pid: settings.pid,
                uid: settings.vid.substring(0, 10),
                vid: settings.vid,
                flow: 0,
                pd: settings.stay_duration,
                sd: settings.stay_duration,
                ts: ts,
                sign: sign
            };
            for (var p in settings.params) {
                data[p] = settings.params[p]
            }
            polyvObject.ajax({
                type: "GET",
        ...
        function makeH5Player(o) {
            return o.each(function() {
                var sourceType;
                var polyvObjectthis = polyvObject(this);
                var polyvObjectsettings = settings;
                var imageUrl = settings.first_image;
                if (settings.loading_bg_img.length > 0) {
                    imageUrl = settings.loading_bg_img
                }
                var div = document.createElement("div");
                div.setAttribute("id", "plv_container");
                var polyvObjectdiv = polyvObject(div);
                polyvObjectdiv.css("background-size", settings.playerWidth * 100 + "%");
                polyvObjectdiv.css("background-repeat", "no-repeat");
                polyvObjectdiv.css("background-position", "center");
                polyvObjectdiv.css("text-align", "left");
                polyvObjectdiv.css("width", settings.width);
                polyvObjectdiv.css("height", settings.height);
                this.appendChild(div);
                if (settings.outdate) {
                    switch (settings.status) {
                    case 50:
                        imageUrl = resourceURL + "/file/images/invalid_sh.jpg";
                        break;
                    case 10:
                        imageUrl = resourceURL + "/file/images/invalid_bm.jpg";
                        break;
                    case 20:
                        imageUrl = resourceURL + "/file/images/invalid_bm.jpg";
                        break;
                    default:
                        imageUrl = resourceURL + "/file/images/invalid.jpg"
                    }
                } else {
                    var video = document.createElement("video");
                    var polyvObjectvideo = polyvObject(video);
                    polyvObjectvideo.attr("webkit-playsinline", "");
                    polyvObjectvideo.css("display", "none");
                    polyvObjectvideo.css("width", "0");
                    polyvObjectvideo.css("height", "0");
                    div.appendChild(video);
                    if (!settings.autoplay) {
                        var buttonImageUrl = resourceURL + "/file/images/video-play.png";
                        var img = polyvObject('<img id="playbutton">');
                        img.attr("src", buttonImageUrl);
                        img.appendTo(polyvObjectdiv);
                        img.one("load",
                        function() {
                            if (parseInt(settings.height) == 0) {
                                var videoheight = parseInt(polyvObjectdiv.css("width")) / settings.ratio;
                                settings.height = videoheight;
                                polyvObjectthis.css("height", videoheight);
                                polyvObjectdiv.css("height", videoheight)
                            }
                            var x = (parseInt(polyvObjectdiv.css("width")) - 121) / 2;
                            var y = (parseInt(polyvObjectdiv.css("height")) - 121) / 2;
                            var width = parseInt(polyvObjectdiv.css("width")) / 3;
                            img.css("left", x + "px");
                            img.css("top", y + "px");
                            img.css("width", "121px");
                            img.css("height", "121px");
                            if (typeof s2j_onPlayerInitOver == "function") {
                                s2j_onPlayerInitOver()
                            }
                            img.css("position", "relative");
                            img.css("opacity", ".8");
                            polyvObjectdiv.bind("click",
                            function() {
                                img.remove();
                                makeVideo(div, video);
                                polyvObjectdiv.unbind("click");
                        ...
                        polyvObjectvideo.attr("autoplay", "");
                        makeVideo(div, video);
                        if (typeof s2j_initVideoPlayer == "function") {
                            console.log(">js .init");
                            s2j_initVideoPlayer()
                    ...
                    settings.j2s_getVideo = function() {
                        return video
                    };
                    var staytimer;
                    video.addEventListener("playing",
                    function() {
                        settings.pid = polyvObject.getPid();
                    ...
                    var curpos = 0;
                    var watch_start_time = settings.flashvars["watchStartTime"];
                    if (!watch_start_time) {
                        watch_start_time = settings.watch_start_time
                    }
                    ...
                    function() {
                        if (watch_start_time && watch_start_time != 0) {
                            video.currentTime = watch_start_time
                        }
                        settings.j2s_getCurrentTime = function() {
                            return video.currentTime
                        };
                        settings.j2s_stayInVideoTime = function() {
                            return settings.stay_duration
                        };
                        settings.j2s_getDuration = function() {
                    ...
                    var watchEndTime = settings.flashvars["watchEndTime"];
                    timeUpdate = function() {
                        curpos = video.currentTime;
                        if (watchEndTime && video.currentTime > watchEndTime) {
                            settings.j2s_stopVideo()
                        }
                    ...
                    function() {
                        if (settings.s2j_onVideoSeeking) {
                            settings.s2j_onVideoSeeking()
                        }
                    });
                    video.addEventListener("seeked",
                    function() {
                        if (settings.s2j_onVideoSeeked) {
                            settings.s2j_onVideoSeeked()
                        }
                    });
                    video.addEventListener("progress",
                    function() {
                        if (settings.s2j_onProgressUpdate) {
                            settings.s2j_onProgressUpdate()
                        }
                    })
                }
                polyvObjectdiv.css("background-image", "url(" + imageUrl + ")");
                if (settings.width.charAt(settings.width.length - 1) == "%") {
                    polyvObjectthis.width(settings.width);
                    polyvObjectthis.height(settings.height)
                }
            })
        }
        return settings
    };
    polyvObject.fn.previewPlayer = createPreviewPlayer;
    polyvObject.fn.videoPlayer = createPlayer;
    polyvObject.fn.showPlayer = createShowPlayer;
    polyvObject.getPid = function getPid() {
        var date = new Date;
        var ts = date.getTime() + "";
        var randnum = parseInt(Math.random() * 1e6 + 1e6) + "";
        return ts + "X" + randnum
    };
    polyvObject.getPlayer = function getPlayer(playId) {
        if (navigator.appName.indexOf("Microsoft") != -1) {
            var reObj = window[playId];
            try {
                if (reObj.length > 0) {
                    return reObj[0]
                } else {
            ...
            return document[playId]
        } else {
            return document[playId]
        }
    };
    polyvObject.playerType = {
        FLASH: "flash",
        HTML: "html",
        INSTALLER: "installer",
        NO_SUPPORT: "nosupport"
    };
    polyvObject.determinePlayerType = function(settings, flashSupport, htmlSupport) {
        if (settings.forceHTML5) {
            return polyvObject.playerType.HTML
        }
        if (settings.forceFlash) {
            return polyvObject.playerType.FLASH
        }
        if (polyvObject.isFirefox() == true) {
            return polyvObject.playerType.FLASH
        }
        if (flashSupport == null && htmlSupport == false) {
            return polyvObject.playerType.NO_SUPPORT
        }
        if (flashSupport != null) {
            if (polyvObject.isFlashVersionSufficient(flashSupport)) {
                return polyvObject.playerType.FLASH
            } else {
                return polyvObject.playerType.INSTALLER
            }
        }
        if (htmlSupport) {
            return polyvObject.playerType.HTML
    ...
    polyvObject.isFlashVersionSufficient = function(flashSupport) {
        if (flashSupport == null) return false;
        if (flashSupport.majorVersion > polyvObject.majorVersion || flashSupport.majorVersion == polyvObject.majorVersion && flashSupport.majorRevision > polyvObject.majorRevision) {
            return true
        }
        return false
    };
    polyvObject.isFirefox = function() {
    ...
    polyvObject.checkHtmlSupport = function() {
        ...
        return videoSupport && canvasSupport && polyvObject.isSupportedHTMLDevice()
    };
    polyvObject.checkLowerDevice = function() {
        var lower = window.screen.height == 960 / 2;
        if (window.devicePixelRatio == 1) {
            lower = true
        }
        return lower
    };
    polyvObject.isIOS = function(pUAString) {
        var types = ["iPad", "iPhone", "iPod"];
        var numTypes = types.length;
        var uaString = pUAString || navigator.userAgent;
    ...
    polyvObject.isSupportedHTMLDevice = function(pUAString) {
    ...
    polyvObject.isAndroid = function() {
        if (navigator.userAgent.match(new RegExp("android", "i"))) {
            return true
        }
        return false
    };
    polyvObject.isIE = window.ActiveXObject != undefined;
    polyvObject.checkFlashSupport = function() {
        var versions = polyvObject.isIE ? polyvObject.checkFlashSupportIE() : polyvObject.checkFlashSupportStandard();
        return versions
    };
    polyvObject.checkFlashSupportIE = function() {
        var versions;
        try {
    ...
    polyvObject.checkFlashSupportStandard = function() {
        var versions;
        var majorVersion;
        var majorRevision;
        var minorRevision;
